...
function Player:init(map)
    ...
    self:positionOnTopOfMaptiles(map)
    ...
    self.animations = {
        ['idle'] = Animation({
            ...
            frames = {
                love.graphics
                    .newQuad(0, 0, 16, 20, self.texture:getDimensions())
            }
        }),
        ['walking'] = Animation({
            texture = self.texture,
            frames = {
                love.graphics.newQuad(128, 0, 16, 20,
                                      self.texture:getDimensions()),
                love.graphics.newQuad(144, 0, 16, 20,
                                      self.texture:getDimensions()),
                love.graphics.newQuad(160, 0, 16, 20,
                                      self.texture:getDimensions()),
                love.graphics.newQuad(144, 0, 16, 20,
                                      self.texture:getDimensions())
            },
        ...
        ['jumping'] = Animation({
            texture = self.texture,
            frames = {
                love.graphics.newQuad(32, 0, 16, 20,
                                      self.texture:getDimensions())
            }
        })
    }
...
function Player:update(dt)
    ...
    self.x = math.min(self.map.tileWidth * self.map.mapWidth, self.x)
...
function Player:positionOnTopOfMaptiles(map)
    self.y = map.tileHeight * ((map.mapHeight - 2) / 2) - self.height
    self.x = map.tileWidth * 0
end
